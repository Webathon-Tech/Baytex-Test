...
| HAProxy is not installed, or the load balancer probe is unhealthy, after a deploy | The proxy subnet reaches the package mirrors only through the firewall, so the install waits for the hub peering and the firewall rule, retrying every minute | Complete the peering and the firewall rule; follow progress with `az vm run-command invoke -g <rg> -n <vm> --command-id RunShellScript --scripts "tail -n 20 /var/log/cloud-init-output.log"` |
